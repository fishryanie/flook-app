Feature: Features in the filters button of the genres page

  @AdminWeb
  Scenario: Verify that when the filters button in the genres page is clicked it will display the full component
  ...
  Scenario Outline: Verify that contains filter mode returns correct results
    ...
    Then I will see a list of genres that contain the key word "<keyWord>"
    Examples:
      |columnTitle|keyWord|
      |Stt        |1      |
      |Tên loại   |am     |
  ...
  Scenario Outline: Verify that equals filter mode returns correct results
    ...
    And I enter the key word "<keyWord>" in value edit text
    Then I will see a list of genres that equal the key word "<keyWord>"
    Examples:
      |columnTitle|keyWord  |
      |Tên loại   |Adventure|
      |Stt        |4        |
  ...
  Scenario Outline: Verify that starts with filter mode returns correct results
    ...
    And I enter the key word "<keyWord>" in value edit text
    Then I will see a list of genres that star with the key word "<keyWord>"
    Examples:
      |columnTitle|keyWord|
      |Tên loại   |Ad     |
      |Stt        |2      |
  ...
  Scenario Outline: Verify that ends with filter mode returns correct results
    ...
    And I enter the key word "<keyWord>" in value edit text
    Then I will see a list of genres that end with the key word "<keyWord>"
    Examples:
      |columnTitle |keyWord  |
      |Stt         |2        |
      |Tên loại    |inh      |
